    ...
    <div className="space-y-3">
      ...
      <ul className="divide-y divide-sep rounded-lg border border-sep">
        {items.map((item) => (
          <li
            key={item.id}
            className={`flex items-start gap-3 p-3 ${item.readAt ? "" : "bg-accent-soft/60/20"}`}
          >
            <span
              aria-hidden
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-sec">{item.body}</p>
              <p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-sec">
