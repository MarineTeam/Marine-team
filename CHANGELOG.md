...
- **Query Monitor**: a WordPress-Query-Monitor-style debug bar shown at the
  ...
  environment variable (case-insensitive) rather than a database-toggled
  plugin like every other optional feature, since it's an ops/dev tool;
  `/admin/query-monitor` shows whether it's currently on but can't flip it.
  Renders for logged-in `ADMIN` users only, even when enabled, and updates
  on every client-side navigation rather than freezing at whichever page
